	...
	if (!isset($_SESSION['idUsuario'])) {
		...
		if ($oper==1) {	//agregar
			if ($status==1) {
				...
				if ($sw_error==0) {
					...
					$query3 = "INSERT INTO cargas_otras_faenas VALUES(NULL,".strtoupper($guia_).",".$descripcion_.",".$patente_.",".$chofer_.",NULL,".$faena_.",".$hoy.", NULL, NULL, NULL, 0, 0, 0);";
					//echo $query3;
		...
		if ($oper==2) {	//acualizar
			if ($status==1) {
				...
				$query3 = "UPDATE cargas_otras_faenas SET descripcion=".$descripcion_.", patente=".$patente_.", faena=".$faena_.", chofer=".$chofer_." WHERE id=".$id.";";
				$result = mysqli_query($conn,$query3);
				//
				$patente_dato = "";
				$rut_chofer_dato = "";
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function agregar( id )
	{
		document.recepcion.oper.value=1;
		document.recepcion.id.value=id;
		document.recepcion.submit();
	}
	function cancelar()
	{
		document.recepcion.oper.value=0;
		document.recepcion.submit();
	}
	function editar( id )
	{
		document.recepcion.oper.value=2;
		document.recepcion.id.value=id;
		document.recepcion.submit();
	}
	function guardar()
	{
		var ret=0;
		if (document.recepcion.guia.value=="") {
			ret=1;
			alert("Debe indicar Guia.");
			document.recepcion.guia.focus();
		} else {
			if (document.recepcion.descripcion.value=="") {
				ret=1;
				alert("Debe ingresar Descripci\363n.");
				document.recepcion.descripcion.focus();
			} else {
				if (document.recepcion.faena.selectedIndex==0) {
					ret=1;
					alert("Debe Indicar Faena.");
					document.recepcion.faena.focus();
				} else {
					if (document.recepcion.patente.selectedIndex==0) {
						ret=1;
						alert("Debe Indicar Patente.");
						document.recepcion.patente.focus();
					} else {
						if (document.recepcion.chofer.selectedIndex==0) {
							ret=1;
							alert("Debe Indicar Chofer.");
							document.recepcion.chofer.focus();
						}
					}
				}
			}
		...
		if (ret==0) {
			document.recepcion.status.value=1;
			document.recepcion.submit();
		}
	}
	function actualizar()
	...
	function borrar( nombre, id )
	{
		if (confirm("\277 Confirma la eliminaci\363n de esta Asignacion: \n" + nombre + " ?")) {
			document.recepcion.id.value = id;
			document.recepcion.oper.value = 3;
			document.recepcion.submit();
		}
	}
	function enlacePaginacion( pagina ) 
	{
		document.recepcion.page.value=pagina;
		document.recepcion.submit();
	}
	function busqueda()
	{
	...
	function recepcionar( imagen, id )
	{
		...
		p0 = id;
		p1 = modo;
		$.post("ayax_actualiza_estado_recepcion.php",{ p0: p0, p1: p1 },function(data){})
		//
		if (modo==1) {
    ...
    <body onLoad="inicio();">
    <form method="post" action="recepcion_doc.php" target="_self" name="recepcion" id="recepcion" onSubmit="return true;">
    <!-- loading animation -->
            ...
            <div class="contentwrapper"><!--Content wrapper-->
                ...
                <div class="heading">
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li class="active" style="color: white;">Recepci&oacute;n Documentos</li>
                    </ul>
                            ...
                            <div class="box">
                                ...
                                <div class="content noPad">
                                    ...
                                    <table class="responsive table table-bordered">
										...
										$query = "SELECT A.*, B.patente AS 'DES_PATENTE', C.nombre, C.apellidos, D.descripcion AS 'DES_FAENA' FROM cargas_otras_faenas A LEFT JOIN patentes B ON A.patente=B.id LEFT JOIN choferes C ON A.chofer=C.id LEFT JOIN faenas D ON A.faena=D.id ".trim($condicion)." ORDER BY A.patente, A.guia ";
										$query .= " $limit";
										$result = mysqli_query($conn,$query);
										//echo $query;
										while ($row = mysqli_fetch_array($result))
										{
											...
											?>
	                                          <tr>
                                            	...
                                            	if ($estadoGuia==0) { 
		                                            if ($row["estado"]==1) { ?>
			                                            <td style="text-align: center;"><a href="#" onClick="recepcionar('img_<?php echo trim($contador);?>', <?php echo trim($row['id']);?>);"><img name="img_<?php echo trim($contador);?>" id="img_<?php echo trim($contador);?>" src="images/si.png" style="cursor: pointer;" title="Recepcionar"></a></td>
			                                        <?php } else { ?>
			                                            <td style="text-align: center;"><a href="#" onClick="recepcionar('img_<?php echo trim($contador);?>', <?php echo trim($row['id']);?>);"><img name="img_<?php echo trim($contador);?>" id="img_<?php echo trim($contador);?>" src="images/no.png" style="cursor: pointer;" title="Recepcionar"></a></td>
			                                        <?php } 
		                                      	} else { ?>
	                                            ...
	                                            <td>
	                                                <div class="controls center"> 
	                                                    <a href="#" title="Agregar" class="tip" onClick="agregar(<?php echo trim($row['id']);?>);"><span class="icon12 entypo-icon-plus-2"></span></a>
