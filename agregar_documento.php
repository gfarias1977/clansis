<?php
	...
	if ($oper==0) {	
		if ($status==1) {
			...
			$documento_="NULL";
			if ($documento != "") {
				$documento_ = "'".strtoupper(trim($documento))."'";
			}
			$query = "SELECT A.* FROM guias_iansa A WHERE A.asignacion_id=".trim($id_asig)." AND A.guia=".$documento_.";";
			...
			//echo $query;
			if ($grabar==1) {
				$query = "INSERT INTO guias_iansa VALUES(NULL,".trim($id_asig).",'".trim($causante2)."',".$documento_.", '', NULL, NULL, 0, 0, NULL, NULL, NULL, 0, NULL, NULL, NULL, 0);";
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	{
		var ret=0;
		if (document.agregar.documento.value=="") {
			alert("Debe indicar DOCUMENTO.");
			document.agregar.documento.focus();
			ret=1;
		}
		if (ret==0) {
			document.agregar.status.value=1;
			document.agregar.submit();
		}
	}
	function salir() {
		parent.cancelar_man(0);
	}
    </script>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" /></head>
      
    <body onLoad="inicio();">
    <form method="post" action="agregar_documento.php" target="_self" name="agregar" id="agregar">
            ...
            <div class='content' style="height: 360px;">
                <div class='form-row row-fluid' style="margin-top: 0px;">
                    <div class='span12'>
                        ...
                        <div class="row-fluid">
                            <span class="span3"><label class="form-label" for="normal">DOCUMENTO</label></span>
                            <input class="span4 mayuscula" style="width: 128px;" id="documento" name="documento" type="text" maxlength="12" value="<?php echo $documento;?>" />
                        </div>
