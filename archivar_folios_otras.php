...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function archivar( imagen, folio )
	{
		...
		p0 = folio;
		p1 = modo;
		$.post("ayax_actualiza_estado_folio_otras.php",{ p0: p0, p1: p1 },function(data){})
		//
		if (modo==1) {
    ...
    <body onLoad="inicio();">
    <form method="post" action="archivar_folios_otras.php" target="_self" name="listado" id="listado" onSubmit="return false;">
    <!-- loading animation -->
    ...
    <div id="wrapper">
		...
		if ($oper==0) {	// Visualizar
			...
			$rows_per_page= 100;
			$tiene_filtro = false;
			$query = "SELECT DISTINCT A.folio, DATE_FORMAT(fecha_rendicion, '%d/%m/%Y') AS 'FechaFolio' FROM cargas_otras_faenas A ";
			if ($f_estado == 0) {
                            ...
                            <div class="box">
                                ...
                                <div class="content noPad">
                                    ...
                                    <table class="responsive table table-bordered">
										...
										if ($orden==0) {
											$query = "SELECT DISTINCT A.folio, DATE_FORMAT(A.fecha_rendicion, '%d/%m/%Y') AS 'FechaFolio', A.fecha_rendicion, A.archivado FROM cargas_otras_faenas A ".$condicion." ORDER BY A.fecha_rendicion DESC ";
										}
										if ($orden==11) {
											$query = "SELECT DISTINCT A.folio, DATE_FORMAT(A.fecha_rendicion, '%d/%m/%Y') AS 'FechaFolio', A.fecha_rendicion, A.archivado FROM cargas_otras_faenas A ".$condicion." ORDER BY A.folio ASC ";
										}
										if ($orden==12) {
											$query = "SELECT DISTINCT A.folio, DATE_FORMAT(A.fecha_rendicion, '%d/%m/%Y') AS 'FechaFolio', A.fecha_rendicion, A.archivado FROM cargas_otras_faenas A ".$condicion." ORDER BY A.folio DESC ";
										}
										if ($orden==21) {
											$query = "SELECT DISTINCT A.folio, DATE_FORMAT(A.fecha_rendicion, '%d/%m/%Y') AS 'FechaFolio', A.fecha_rendicion, A.archivado FROM cargas_otras_faenas A ".$condicion." ORDER BY A.fecha_rendicion ASC ";
										}
										if ($orden==22) {
											$query = "SELECT DISTINCT A.folio, DATE_FORMAT(A.fecha_rendicion, '%d/%m/%Y') AS 'FechaFolio', A.fecha_rendicion, A.archivado FROM cargas_otras_faenas A ".$condicion." ORDER BY A.fecha_rendicion DESC ";
										}
