<?php
	...
	if ($oper==0) {	
		if ($status==1) {
			...
			$documento_="NULL";
			if ($comentario != "") {
				$comentario_ = "'".trim($comentario)."'";
			}
			if ($grabar==1) {
				$query3 = "UPDATE guias_iansa SET comentario=".$comentario_." WHERE id=".$id_guia.";";
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function inicio() {
		<?php if ($grabar==1) { 
			echo "parent.cancelar_com(0);\n";
		}?>
	}
	function guardar()
	{
		var ret=0;
		if (document.comentario.comentario.value=="") {
			alert("Debe indicar COMENTARIO.");
			document.comentario.comentario.focus();
			ret=1;
		}
		if (ret==0) {
			document.comentario.status.value=1;
			document.comentario.submit();
		}
	}
	function salir() {
		parent.cancelar_com(0);
	}
    </script>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" /></head>
      
    <body onLoad="inicio();">
    <form method="post" action="agregar_comentario.php" target="_self" name="comentario" id="comentario">
                ...
                <div class='form-row row-fluid' style="margin-top: 0px;">
                    <div class='span12'>
                        <div class="row-fluid" style="margin-top: 0px;">
                            <span class="span3"><label class="form-label" for="controls" style="text-align: left; font-weight: bold;">Comentario</label></span>
                            <textarea class="span4" id="comentario" name="comentario" rows="5" cols="10" maxlength="2000" /><?php echo $comentario;?></textarea>
                        </div>
                        <div class='row-fluid' style="margin-top: 80px; text-align: right; margin-right: 20px; margin-bottom: 0px;">
                            <a href='#' onClick="salir();" target='_blank'><span class='btn btn-danger'>Cerrar Ventana</span></a>
                            <a href='#' onClick="guardar();" target='_blank'><span class='btn btn-success'>Guardar Comentario</span></a>
                        </div>
