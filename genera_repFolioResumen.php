<?php
function GeneraReporte2( $name, $fecha_hoy ) {
...
$fec1 = "";
if ($fecha_hoy <> "") {
	$fec1 = f_retorna_fecha_mysql( $fecha_hoy );
}
...
$lineas = 46;
$items = 0;
$query = "SELECT A.*, B.patente, B.ruta, B.nombre_destinatario, B.rut_chofer, B.nombre_chofer, B.causante FROM guias_iansa A LEFT JOIN cargas_iansa B ON A.asignacion_id=B.id WHERE (A.estado = 1 AND (DATE_FORMAT(B.fecha, '%Y-%m-%d') = ".$fec1.")) ORDER BY A.folio, A.guia ASC; "; 
//echo $query;
...
if ($result) {
	while ($row = mysqli_fetch_array($result))
	{
		if ($lineas > 45) {
			...
			$pdf->Cell(0,0,"RENDICION COMPLETAS - RESUMEN DIA: ".trim($fecha_hoy),0,1,'C');
			$pdf->Ln(18);
		...
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(3,11," ",0,0,'L');
	    $pdf->Cell(25,11,trim($folio),'LB',0,'C');
...
$lineas = 46;
$items = 0;
$query = "SELECT A.*, B.patente, B.ruta, B.nombre_destinatario, B.rut_chofer, B.nombre_chofer, B.causante FROM guias_iansa A LEFT JOIN cargas_iansa B ON A.asignacion_id=B.id WHERE (A.estado = 2 AND (DATE_FORMAT(B.fecha, '%Y-%m-%d') = ".$fec1.")) ORDER BY A.folio, A.guia ASC; "; 
//echo $query;
$result = mysqli_query($conn,$query);
if ($result) {
	while ($row = mysqli_fetch_array($result))
	{
		if ($lineas > 45) {
			...
			$pdf->Image("images/logo1.png",30,16,130,0,'');
			$pdf->SetFont('Arial','B',11);
			$pdf->Cell(0,0,"RENDICION PENDIENTES - RESUMEN DIA: ".trim($fecha_hoy),0,1,'C');
			$pdf->Ln(18);
			// Line(x1,y1,x2,y2) x1=col1 x2=col2 y1=row1 y2=row2
			$pdf->Line(30,45,760,45);
			$pdf->SetFont('Arial','',7);
			...
			$pdf->Cell(60,11,"Causante",1,0,'C');
			$pdf->Cell(60,11,"Guia",1,0,'C');
			$pdf->Cell(30,11,"Pallets",1,0,'C');
			$pdf->Cell(190,11,"Comentarios",1,1,'L');
			$pdf->Ln(1);
			$lineas = 10;
		}
		$items++;
	    ...
	    $pdf->Cell(60,11,trim($causante),'LB',0,'C');
	    $pdf->Cell(60,11,trim($guia),'LBR',0,'C');
	    $pdf->Cell(30,11,trim($pallets),'LBR',0,'C');
	    $pdf->Cell(190,12,trim($comentario),'LBR',1,'L');
	    $pdf->Ln(1);
	    $lineas = $lineas + 1;
	}
	mysqli_free_result($result);
