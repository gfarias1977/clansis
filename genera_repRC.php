<?php
function GeneraReporte( $name, $modo, $folio ) {
...
$items = 0;
if ($modo == 1) {
	$query = "SELECT A.*, B.patente, B.ruta, B.nombre_destinatario, B.rut_chofer, B.nombre_chofer, B.causante FROM guias_iansa A LEFT JOIN cargas_iansa B ON A.asignacion_id=B.id WHERE (A.estado = 1 AND A.estadoRC = 0) AND A.preselec = 1 ORDER BY A.guia ASC; "; 
	//echo $query;
...
if ($modo==2) {
	$lineas = 36;
	$items = 0;
	$query = "SELECT A.*, B.patente, B.ruta, B.nombre_destinatario, B.rut_chofer, B.nombre_chofer, B.causante FROM guias_iansa A LEFT JOIN cargas_iansa B ON A.asignacion_id=B.id WHERE (A.estado = 2 AND A.estadoRC = 0) AND A.preselec = 1 ORDER BY A.guia ASC; "; 
	//echo $query;
	$result = mysqli_query($conn,$query);
	if ($result) {
		while ($row = mysqli_fetch_array($result))
