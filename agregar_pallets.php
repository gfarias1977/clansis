<?php
	...
	if ($oper==0) {	
		if ($status==0) {
			$f_chep = "";
			$f_iansa = "";
			$query = "SELECT * from guias_iansa Where id=".trim($id_guia);
			$result = mysqli_query($conn,$query);
			while ($row = mysqli_fetch_array($result))
			{
				if (!is_null($row["IANSA"])) {
					$f_iansa = trim($row["IANSA"]);
				}
			}
		...
		if ($status==1) {
			...
			if ($f_iansa=="") {
				...
				$f_iansa_ = str_replace(",",".",$f_iansa);
			}
			$query = "UPDATE guias_iansa SET IANSA=".trim($f_iansa_)." WHERE id=".trim($id_guia).";";
			$result = mysqli_query($conn,$query);
			//
			$oper=0;
			$status=0;
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function inicio() {
		<?php if ($grabar==1) { 
			echo "id = document.notas.id_guia.value;";
			echo "indice = document.notas.indice.value;";
			echo "parent.cancelar_pal(0, id, indice);\n";
		}?>
	}
	function guardar()
	{
		var ret=0;
		if (ret==0) {
			if (document.notas.f_iansa.value!="") {
				var valor="";
				valor = document.notas.f_iansa.value;
				valor = valor.replace(",", ".");
				if (isNaN(valor)) {
					ret=1;
					alert("Debe indicar un Valor Numerico para PALLETS.");
					document.notas.f_iansa.focus();
				} else {
					valor = parseInt(valor);
					document.notas.f_iansa.value=valor;
				}
			}
		}
		if (ret==0) {
			document.notas.status.value=1;
			document.notas.submit();
		}
	}
	function salir() {
		id = document.notas.id_guia.value;
		indice = document.notas.indice.value;
		parent.cancelar_pal(0, id, indice);
	}
	function val_numeros(objeto){
		if(objeto.value != ""){
			if(check_number(objeto.value)){
				...
				alert("Debe ingresar un valor num\351rico.");
				objeto.value="";
				return;
			}
		}
    ...
    <body onLoad="inicio();">
    <form method="post" action="agregar_pallets.php" target="_self" name="notas" id="notas">

	<?php
	if ($oper==0) { 
		$query = "SELECT A.* FROM guias_iansa A WHERE A.id=".trim($id_guia);
		//echo $query;
		$result = mysqli_query($conn,$query);
		while ($row = mysqli_fetch_array($result))
		{
                ...
                <div class='form-row row-fluid' style="margin-top: 0px;">
                    <div class='span12'>
                        <div class="row-fluid" style="margin-top: 0px;">
                            <span class="span3" style="margin-left: 20px;"><label class="form-label" for="controls" style="text-align: left; font-weight: bold;">PALLETS</label></span>
	                        <span class="span5"><input id="f_iansa" name="f_iansa" type="text" value="<?php echo $f_iansa;?>" maxlength="3" onblur="val_numeros(this);" style="font-size: 11pt; width: 100px; margin-left: 20px;" /></span>
                        </div>
                        <div class='row-fluid' style="margin-top: 230px; text-align: right; margin-right: 40px; margin-bottom: 0px;">
                            <a href='#' onClick="salir();" target='_blank'><span class='btn btn-danger'>Cerrar Ventana</span></a>
                            <a href='#' onClick="guardar();" target='_blank'><span class='btn btn-success'>Guardar Datos</span></a>
                        </div>
